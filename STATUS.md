...
- Mailchimp integration details — awaiting Anna's decision (sent to her 2026-08-13, see Notes)
- Testimonials "Read all reviews" link is a dead `#` placeholder — no testimonials page exists
- Footer social links and Privacy Policy link are dead `#` placeholders on all pages — awaiting Anna's Instagram/Facebook URLs and privacy policy text (sent to her 2026-08-13)
- About page: standalone `about.html` not yet built — awaiting Anna's decision on whether it's needed
- `retreat-october.html`'s "What's Included" list was carried over from the old generic October Retreat copy — awaiting Anna's real day-by-day rundown (sent to her 2026-08-13)
- **Pre-launch technical checklist (Harvey handling, not Anna)** — none of these exist yet: favicon, `robots.txt`, `sitemap.xml`, Open Graph/Twitter Card meta tags (no link preview when shared on social), `CNAME` file (needed for the custom domain once hosting is decided), schema.org structured data. Also: `landscapevid.mp4` (98MB) and `damhorsevid.mp4` (60MB) are large enough to seriously hurt mobile load time and Core Web Vitals — need compressing before launch. No `prefers-reduced-motion` handling anywhere (ties to the parallax/Hard-Rule item below). Full click-through/form-submission/mobile/Safari test pass hasn't been done.
